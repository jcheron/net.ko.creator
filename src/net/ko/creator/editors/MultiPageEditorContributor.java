...
public class MultiPageEditorContributor extends MultiPageEditorActionBarContributor {
	...
	private void createActions() {
		connectToDbAction = new Action() {
			public void run() {
				...
				if(editor!=null){
					...
					((MultiPageEditor)(editor)).gotoPage(1);
				}
			}
		};
		setActionAttributes(connectToDbAction, "connectToDb", "Connexion à la base de données", "Connexion à la base de données", Images.DB_CONNECT);
		...
		setActionAttributes(selectAllTables, "selectAllTables", "Dé/Sélectionner toutes les tables", "Dé/Sélectionner toutes les tables", Images.CHECK);
		...
		setActionAttributes(generateClasses, "generateAllClasses", "Générer", "générer les classes", Images.ORM);
	}
	private void setActionAttributes(Action a,String id,String text,String toolTip,String image){
		a.setId(id);
		a.setText(text);
		a.setToolTipText(toolTip);
		a.setImageDescriptor(ImageDescriptor.createFromURL(Platform.getBundle(Activator.PLUGIN_ID).getResource(image)));
	}
