package net.ko.creator.controls.linecmb;
 ...
 * </dl>
 */
public final class LineStyleCombo extends Composite {
	...
	Font font;
	Color elementColor = ColorConstants.blue;
	int elementSize = 2;
	int elementLineStyle = SWT.BORDER_SOLID;
	 ...
	 * @see Widget#getStyle()
	 */
	public LineStyleCombo(Composite parent, int style) {
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (LineStyleCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == LineStyleCombo.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
		...
		int[] textEvents = { SWT.KeyDown, SWT.KeyUp, SWT.Modify, SWT.MouseDown, SWT.MouseUp, SWT.Traverse, SWT.FocusIn, SWT.Paint };
	...
	Label getAssociatedLabel() {
		...
		for (int i = 0; i < siblings.length; i++) {
			if (siblings[i] == LineStyleCombo.this) {
	...
	void textEvent(Event event) {
		...
		case SWT.Paint:
			GC gc = event.gc;
			gc.setForeground(elementColor);
			gc.setLineStyle(elementLineStyle);
			gc.setLineWidth(elementSize);
			gc.drawLine(0, text.getSize().y / 2, text.getSize().x, text.getSize().y / 2);
			break;
		}
	}

	public void elementSelected(int index) {
		setElementLineStyle(index + 1);
	}
	...
	public int getElementLineStyle() {
		return elementLineStyle;
	}

	public void setElementLineStyle(int elementLineStyle) {
		this.elementLineStyle = elementLineStyle;
		text.redraw();
	}
}
